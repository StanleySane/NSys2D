// ComplexMatr.cpp: implementation of the CComplexMatr class.
//
//////////////////////////////////////////////////////////////////////
...
CComplexMatr::CComplexMatr(int ny, int nx)
{
	Data=0;
	SizeX=SizeY=0;
...
Complex* CComplexMatr::operator [](int n)
{
	ASSERT(Data);
	ASSERT(n<SizeY);
...
int CComplexMatr::IsEmpty()
{
	if (Data) return 0;
	return 1;
}

void CComplexMatr::Clear()
{
	if (Data)
	{
...
void CComplexMatr::ReSize(int ny, int nx)
{
	ASSERT(nx>0);
	ASSERT(ny>0);
	...
	Data=new Complex*[SizeY];
	for (int i=0;i<SizeY;i++)
		Data[i]=new Complex[SizeX];
	ClearData();
}

void CComplexMatr::ClearData()
{
	for (int i=0;i<SizeY;i++)
...
CComplexMatr::CComplexMatr(CComplexMatr & matr)
{
	Data=0;
	SizeX=SizeY=0;
...
CComplexMatr CComplexMatr::operator *(CComplexMatr & matr)
{
	ASSERT(SizeX==matr.SizeY);
	CComplexMatr Matr(SizeY,matr.SizeX);
...
void CComplexMatr::SaveToFileRe(char * name)
{
	...
	for (int i=0; i<SizeY;i++)
	{
		for (int j=0;j<SizeX;j++)
		{
			...
			double n1=Data[i][j].Re();
			OutMatr << (fabs(n1)<10E-10?0:n1) << "  ";
		}
		OutMatr << endl;
	}
}

void CComplexMatr::SaveToFileIm(char * name)
{
	fstream OutMatr(name,ios::out);
	OutMatr << "Матрица    " << 
		"[" << SizeY << ";" << 
...
CComplexMatr CComplexMatr::GetInvert(int &flag)
{
	ASSERT(SizeX==SizeY);

	CComplexMatr mC(SizeY,SizeX), mU(SizeY,SizeX), mR(SizeY,SizeX);
	mR.SetIdentity();
...
int CComplexMatr::SolveSystem(CComplexMatr & mR, CComplexMatr & mU)
{
	...
	for(int i=0;i<(SizeY-1);i++)
	{
	  ...
	  {
		 if (Len(Data[j][i]) > 1.0e-25)
		 {
			 if (Len(Data[j-1][i]) <1.0e-25)
			 {
...
/*	for (int d=0;d<SizeY;d++)
		if (Len(Data[d][d])<1E-20) return 0;*/
	...
	for (int t=0;t<mR.SizeX;t++)
	{
		for (int i=SizeY-1;i>=0;i--)
		{
			...
			mU[i][t]/=((Len(Data[i][i])<1E-25)?1:Data[i][i]);
		}
	}
	return 1;
}

void CComplexMatr::SetIdentity()
{
	ASSERT(SizeX==SizeY);
...
CComplexMatr& CComplexMatr::UnitMatr(CMatr & matrRe, CMatr & matrJm)
{
	ASSERT((matrRe.SizeX==matrJm.SizeX)&&(matrRe.SizeY==matrJm.SizeY));
	
	Clear();
	ReSize(matrRe.SizeY,matrRe.SizeX);
	for (int i=0;i<SizeY;i++)
		for (int j=0;j<SizeX;j++)
			Data[i][j]=Complex(matrRe[i][j],matrJm[i][j]);
